...
OUTPUT_FOLDER = 'output'
OUTPUT_FILE = 'anomaly_report.json'  # Filename from anomaly_utils.py
UPLOAD_FOLDER = 'uploads'
os.makedirs(UPLOAD_FOLDER, exist_ok=True)
...
def clear_mongo_collection():
    """Clear all documents in the MongoDB collection."""
    try:
        collection.delete_many({})  # Clear the collection
        print("Cleared all documents from MongoDB collection.")
    except Exception as e:
        print(f"Error clearing MongoDB collection: {str(e)}")

def run_anomalies_script(file_path):
    """Run anomalies.py script asynchronously after file upload."""
    try:
        schema_path = "./Schema.json"  # Adjust this path if needed
        command = ['python', 'anomaly_utils.py', file_path, schema_path]
        subprocess.run(command, check=True)
        print(f"Anomalies detection completed for file: {file_path}")
        upload_to_mongodb()
...
def upload_file():
    try:
        ...
        # Clear MongoDB before uploading new data
...
def upload_to_mongodb():
    ...
    try:
        ...
        # Insert into MongoDB
        result = collection.insert_one(document)
        print(f"Data uploaded to MongoDB successfully. Document ID: {result.inserted_id}")
